...
class PdfController extends Controller
{
    ...
    public function deletePdfs(Request $request)
    {
        try {
            ...
            $request->validate([
                'ids' => 'required|array',
                'ids.*' => 'exists:pdfs,id', // Validate each ID exists in the 'pdfs' table
            ]);

            $pdfIds = $request->input('ids');

            // Retrieve PDFs from the database
            $pdfs = Pdf::whereIn('id', $pdfIds)->get();

            // Delete PDFs from storage and database
            foreach ($pdfs as $pdf) {
                // Delete from storage
            ...
            return response()->json(['success' => 'PDFs deleted successfully'], 200);

        } catch (Exception $e) {
            return response()->json(['error' => 'Failed to delete PDFs: ' . $e->getMessage()], 500);
        }
    }
